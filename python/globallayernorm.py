    ...
    def __init__(self, dim: int, shape: int, eps: float = 1e-8, elementwise_affine: bool = True):
        ...
        if self.elementwise_affine:
            if shape == 3:
                ...
                raise ValueError(f"Unsupported shape: {shape}. Only 3 and 4 are supported.")
        else:
            self.weight = None
            self.bias = None
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        if x.ndim == 3:
            ...
            if self.elementwise_affine:
                ...
                w = mx.reshape(self.weight.squeeze() if self.weight.ndim > 1 else self.weight, (1, -1, 1))
                b = mx.reshape(self.bias.squeeze() if self.bias.ndim > 1 else self.bias, (1, -1, 1))
            ...
            if self.elementwise_affine:
                # Optimize broadcasting for weight and bias
                # Reshape weight and bias once for efficient broadcasting
                w = mx.reshape(self.weight.squeeze() if self.weight.ndim > 1 else self.weight, (1, -1, 1, 1))
                b = mx.reshape(self.bias.squeeze() if self.bias.ndim > 1 else self.bias, (1, -1, 1, 1))
                # Fuse normalization and affine transformation
                x = w * (x - mean) / mx.sqrt(var + self.eps) + b
            else:
                x = (x - mean) / mx.sqrt(var + self.eps)
